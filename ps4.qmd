...
categories[6] # <li class="display-inline-block usa-tag text-no-lowercase text-base-darkest bg-base-lightest margin-right-1">Criminal and Civil Actions</li>
...
cat1 = categories[6].text
cat1

link1 = links[5].get('href')
link1 #not the full link rn
...
data_date = []
for i in dates[14:]: 
  data_date.append([i.text]) 
```
```{python}
# get rid of any weird accidental values
#data_titles = data_titles[0:20] #20 rows

data_cats # 23 rows looks good as is
data_links
# create the dataframe

#df = pd.DataFrame({'title': data_titles, 'category': data_cats, 'link': data_links, 'date': data_date})
